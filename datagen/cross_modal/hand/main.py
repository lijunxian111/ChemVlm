...
files = glob('/mnt/hwfile/ai4chem/zhangdi/DECIMER_HDM_Dataset_Images/*')
...
with open('/mnt/hwfile/ai4chem/zhangdi/DECIMER_HDM_Dataset_SMILES.tsv') as f:
    for line in f:
        index,smiles = line.strip().split('\t')
        labels[index] = smiles

def get_label(file):
    index = file.replace('/mnt/hwfile/ai4chem/zhangdi/DECIMER_HDM_Dataset_Images/','').replace('.png','')
    return labels[index]
