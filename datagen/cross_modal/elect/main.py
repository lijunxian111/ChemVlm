...
files = glob('/mnt/hwfile/ai4chem/zhangdi/syn_mole_images/synthetic/*/*.png',recursive=True)
...
for csv in glob('/mnt/hwfile/ai4chem/zhangdi/syn_mole_images/synthetic/*.csv'):
    with open(csv) as f:
        for line in f:
...
def get_label(file):
    index = file.replace('/mnt/hwfile/ai4chem/zhangdi/syn_mole_images/','')
    return labels[index]
