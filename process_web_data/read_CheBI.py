...
def gen_smiles_dataset(data, img_store_path: str):
    ...
    for index, line in tqdm(enumerate(data), desc='gen smiles'):
        ...
        cid_num = line['CID']
        image_np = cv2.imread('/CheBI/image/'+f'CID_{cid_num}.png')
...
if __name__ == "__main__":
    ...
    """
    raw_data = read_data('CheBI/test.csv')
    smiles_prompt_list = gen_smiles_dataset(raw_data, 'cheBI/test/')
    caption_prompt_list = gen_caption_dataset(raw_data, 'cheBI/test/')
    iupac_prompt_list = gen_iupac_dataset(raw_data, 'cheBI/test/')

    total_list = smiles_prompt_list+caption_prompt_list+iupac_prompt_list
    write_total_data(total_list, 'datagen/cheBI_test.jsonl')
